...
- ADRs in `docs/adr/` explain major design decisions.

## Anti-Patterns

- Don't add platform checks to `vibe-engine`. Keep it a pure function.
- Don't hand-edit `src/vibe-engine-wasm/`. Regenerated by wasm-pack.
- Don't duplicate settings between React and Rust. Single source in `vibeStore.ts`.
